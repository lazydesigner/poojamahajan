...
$array_of_city = ['lucknow','delhi','goa','gurugram','udaipur','agra','ghaziabad','indore','jaipur','kota','mumbai','visakhapatnam','rishikesh','bhopal','dehradun','manali','kanpur','jabalpur'];

?>
<?php
include '../init.php';

$uri = explode('/category/', $_SERVER['REQUEST_URI']);

print_r($uri).'<br>';
echo str_replace('-',' ',$uri[1]).'<br>';

$w = str_replace('-',' ',$uri[1]);
foreach($array_of_city as $city){
    if(strpos($w, $city)){
        echo $city.'<br>';
    }else{
        echo 'Not Found'.'<br>';
